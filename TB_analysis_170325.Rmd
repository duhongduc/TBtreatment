...
dat$outcome2<-NA
dat$outcome2[dat$outcome=="Completed"]<-0
...
wb <- createWorkbook()
addWorksheet(wb, "TB Culture Data")

# Define colors for different culture statuses
color_mapping <- c(
  "Positive" = "#FF6961",   # Red
  "Negative" = "#77DD77",   # Green
  "No sputum" = "#FFD700"   # Yellow
)

# Write data to Excel
writeData(wb, "TB Culture Data", df_long, startCol = 1, startRow = 1, colNames = TRUE)

# Apply color formatting
for (i in 2:nrow(df_long) + 1) {  # Start from row 2 (skip header)
  status <- df_long$Culture[i - 1]  # Adjust index
  if (status %in% names(color_mapping)) {
    addStyle(
      wb, "TB Culture Data",
      style = createStyle(fgFill = color_mapping[status]),
      rows = i, cols = 3, gridExpand = TRUE
    )
  }
}

# Save the file
saveWorkbook(wb, "output/TB_Culture_Data.xlsx", overwrite = TRUE)
