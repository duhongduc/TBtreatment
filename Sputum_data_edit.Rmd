...
```{r}
culture_df_long <- mini_data1[, 1:28] %>%
  pivot_longer(cols = 3:28, names_to = "TimePoint", values_to = "Culture") %>%
  mutate(Regimen = factor(Regimen, levels = c("6M", "8M", "9M", "20M"))) %>% 
  arrange(Regimen, studycode) %>%
  mutate(TimePoint = str_remove(TimePoint, "Culture\\."))

plot1.1 <- ggplot(culture_df_long,
       aes(x = TimePoint, y = studycode, fill = Culture)) +
  geom_tile() +
  # scale_fill_identity() +
...
```{r}
row<-c("red", "yellow", "blue", "red", "blue", "blue")
names(row)<-c("T01", "T02", "T03", "T04", "T05", "T06")
find_first_culture_conversion(row)
```
